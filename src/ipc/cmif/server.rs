...
pub fn read_command_from_ipc_buffer(ctx: &mut CommandContext) -> CommandType {
    unsafe {
        ...
        let data_size = (*command_header).get_data_word_count() * mem::size_of::<u32>() as u32;
...
pub fn write_command_response_on_ipc_buffer(ctx: &mut CommandContext, command_type: CommandType, data_size: u32) {
    unsafe {
        ...
        *command_header = CommandHeader::new(command_type as u32, ctx.send_statics.len() as u32, ctx.send_buffers.len() as u32, ctx.receive_buffers.len() as u32, ctx.exchange_buffers.len() as u32, data_word_count, 0, ctx.receive_statics.len() as u32, has_special_header);
        ...
        ipc_buf = ipc_buf.offset((data_word_count * mem::size_of::<u32>() as u32) as isize);
...
pub fn read_request_command_from_ipc_buffer(ctx: &mut CommandContext) -> Result<(u32, DomainCommandType, DomainObjectId)> {
    unsafe {
        ...
        if ctx.object_info.is_domain() {
            ...
            ctx.in_params.data_size -= mem::size_of::<DomainInDataHeader>() as u32;
        ...
        if ctx.in_params.data_size >= DATA_PADDING {
            ...
            if ctx.in_params.data_size >= mem::size_of::<DataHeader>() as u32 {
                result_return_unless!((*data_header).magic == IN_DATA_HEADER_MAGIC, result::ResultInvalidInHeader);

                rq_id = (*data_header).value;
                data_offset = data_header.offset(1) as *mut u8;
                ctx.in_params.data_size -= mem::size_of::<DataHeader>() as u32;
            }
        }
...
pub fn write_request_command_response_on_ipc_buffer(ctx: &mut CommandContext, result: ResultCode, request_type: CommandType) {
    unsafe {
        ...
        if ctx.object_info.is_domain() {
            data_size += (mem::size_of::<DomainOutDataHeader>() + mem::size_of::<DomainObjectId>() * ctx.out_params.objects.len()) as u32;
        }
...
pub fn read_control_command_from_ipc_buffer(ctx: &mut CommandContext) -> Result<ControlRequestId> {
    unsafe {
        ...
        Ok(mem::transmute(control_rq_id))
    }
}
