...
pub fn write_request_command_on_ipc_buffer(ctx: &mut CommandContext, request_id: Option<u32>, domain_command_type: DomainCommandType) {
    unsafe {
        ...
        if ctx.object_info.is_domain() {
            data_size += (mem::size_of::<DomainInDataHeader>() + mem::size_of::<DomainObjectId>() * ctx.in_params.objects.len()) as u32;
        }

        data_size = (data_size + 1) & !1;
        let out_pointer_sizes_offset = data_size;
        data_size += (mem::size_of::<u16>() * ctx.in_params.out_pointer_sizes.len()) as u32;
...
pub fn read_request_command_response_from_ipc_buffer(ctx: &mut CommandContext) -> Result<()> {
    unsafe {
        ...
        data_offset = data_offset.offset(mem::size_of::<DataHeader>() as isize);
        result_return_unless!((*data_header).magic == OUT_DATA_HEADER_MAGIC, result::ResultInvalidOutRawSize);
        ResultCode::new((*data_header).value).to(())?;
