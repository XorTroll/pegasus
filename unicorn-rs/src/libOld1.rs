...
impl<'a> UnicornHandle<'a> {
    ...
    pub fn mem_read_val<T>(&self, address: u64) -> Result<T, uc_error> {
        ...
        let err = unsafe { ffi::uc_mem_read(self.inner.uc, address, &mut t as *mut T as *mut u8, core::mem::size_of::<T>()) };
        if err == uc_error::OK {
            Ok(t)
        } else {
            Err(err)
    ...
    pub fn mem_write_val<T>(&self, address: u64, val: T) -> Result<(), uc_error> {
        let err = unsafe { ffi::uc_mem_write(self.inner.uc, address, &val as *const T as *const u8, core::mem::size_of::<T>()) };
        if err == uc_error::OK {
            Ok(())
        } else {
            Err(err)
    ...
    pub fn reg_write<U>(&mut self, regid: i32, value: U) -> Result<(), uc_error> {
        let err =
            unsafe { ffi::uc_reg_write(self.inner.uc, regid, &value as *const _ as *const c_void) };
        if err == uc_error::OK {
            Ok(())
        } else {
            Err(err)
    ...
    pub fn reg_read<U>(&self, regid: i32) -> Result<U, uc_error> {
        ...
        let err =
            unsafe { ffi::uc_reg_read(self.inner.uc, regid, &mut value as *mut _ as *mut c_void) };
        if err == uc_error::OK {
            Ok(value)
        } else {
            Err(err)
